	    ...
	    */
	    ...
	    _createClass$9(Mesh$$1, [{
	        ...
	        */
	         ...
	         *
	         * @method genereateMesh
	         */

	    }, {
	        key: 'genereateMesh',
	        value: function genereateMesh() {
	...
	*/
	...
	var OBJ = function () {
	    ...
	    _createClass$35(OBJ, null, [{
	         ...
	         */
	        value: function parseMTL(data) {

	            var material = new Mobilizing.Material({ type: "phong" });
	            ...
	            lines.forEach(function (line) {
	                ...
	                switch (words[0]) {
	                    ...
	                    //ambient term
	                    case "Ka":
	                        material.setEmissiveColor(new Mobilizing.Color(words[1], words[2], words[3]));
	                        break;
	                    case "Kd":
	                        //diffuse
	                        material.setColor(new Mobilizing.Color(words[1], words[2], words[3]));
	                        break;

	                    case "Ks":
	                        //specular
	                        material.setSpecularColor(new Mobilizing.Color(words[1], words[2], words[3]));
	        ...
	        key: 'parseOBJ',
	        value: function parseOBJ(data, material) {
	            var text = data;

	            var objMesh = void 0;

	            //debug.log("create material");
	            objMesh = new Mesh$1({ primitive: "custom" });

	            if (material) {
	                objMesh.material = material;
	            } else {
	                objMesh.material = new Material({ type: "phong" });
	            }
	            ...
	            lines.forEach(function (line) {
	                ...
	                switch (words[0]) {
	                    ...
	                    case "v":
	                        //debug.log("Vertice")
	                        objMesh.pushVertex(new Vector3$1(words[1], words[2], words[3]));
	                        break;
	            ...
	            //p.updateMaterial();
	            /*debug.log("callback called : ", OBJ.callBack);
	            OBJ.callBack(p);*/
	    ...
	    _createClass$32(Loader, [{
	        ...
	        */
	        ...
	        value: function loadOBJ(params) {
	            ...
	            var url = params.url;
	            var onStart = getOrDefault(params, "onStart", null);
	            var _onLoad = getOrDefault(params, "onLoad", null);
	            var onError = getOrDefault(params, "onError", function (err) {
	                console.log("error", err);
	            });
	            var loadMtl = getOrDefault(params, "loadMtl", false);

	            var baseUrl = url.replace(/[^/]*$/, "");
	            console.log("baseUrl", baseUrl);

	            //get the obj text file content
	            var text = this.load(Object.assign({}, params, {
	                responseType: "text",

	                "onLoad": function onLoad(objText) {

	                    var mesh = void 0,
	                        material = void 0,
	                        texture = void 0;
	                    //search for mtl
	                    var hasMtllib = void 0;

	                    if (loadMtl) {
	                        var regex = /(mtllib\s)(.*)/gm;
	                        hasMtllib = regex.exec(objText);

	                        console.log("found mtl", hasMtllib);
	                    }

	                    if (hasMtllib) {

	                        var mtllib = baseUrl + hasMtllib[hasMtllib.length - 1];

	                        //load mtl file
	                        _this5.load(Object.assign({}, { url: mtllib, onError: function onError(err) {
	                                console.log("error", err);
	                            } }, {
	                            responseType: "text",
	                            ...
	                            //search for textures
	                            "onLoad": function onLoad(mtlText) {

	                                //find all textures and load them
	                                var regex = /(map_Kd\s)(.*)/gm;
	                                var textures = regex.exec(mtlText);
	                                console.log("textures", textures);

	                                if (textures) {
	                                    //load the textures FIXME : should load all textures!
	                                    //don't forget to add the base url for mtl file to be found

	                                    var mtlUrl = baseUrl + textures[textures.length - 1];

	                                    texture = _this5.loadTexture({ url: mtlUrl,
	                                        onLoad: function onLoad(tex) {

	                                            material = OBJ.parseMTL(mtlText);
	                                            material.setTexture(tex);

	                                            //parse geometry
	                                            mesh = OBJ.parseOBJ(objText, material);
	                                            _onLoad(mesh); //callback with resulting mesh
	                                        } });
	                                } else {
